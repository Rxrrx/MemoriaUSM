...
namespace Clobscode
{
	...
	class Services {
        ...
        static bool WriteVTK(std::string name, FEMesh &output){
            ...
            fprintf(f,"\nCELL_DATA %i\n",(int)elements.size());
            fprintf(f,"SCALARS cell_type int 1\n");
            fprintf(f,"LOOKUP_TABLE default\n");
            for (unsigned int i=0; i<elements.size(); i++) {
                unsigned int np = elements[i].size();
                if      (np == 4) fprintf(f,"10\n");
                else if (np == 5) fprintf(f,"14\n");
                else if (np == 6) fprintf(f,"13\n");
                else if (np == 8) fprintf(f,"12\n");
                else if (np > 8)  fprintf(f,"42\n");
            }
